...
# グラフ画像を保存するフォルダ
OUTPUT_FOLDER = "psd_graphs_1_x0=0.02/"
...
def plot_time_averaged_spectrum(csv_filepath, output_image_path):
    try:
        ...
        if max_val > 0:
            # 最大値で割って、最大を1.0にする
            mean_spectrum_norm = mean_spectrum / max_val
        else:
            mean_spectrum_norm = mean_spectrum

        # 5. dB変換 (対数スケール)
        # log10(0)を防ぐため微小値を加える
        mean_spectrum_db = 10 * np.log10(mean_spectrum_norm + 1e-10)

        # 6. グラフプロット
        plt.figure(figsize=(10, 6))
        plt.plot(f, mean_spectrum_db, linewidth=1.5, color='blue')
        
        plt.title(f"Normalized Time-Averaged Psd\n{os.path.basename(csv_filepath)}")
        plt.xlabel("Frequency [Hz]")
        plt.ylabel("Normalized Power [dB]")
        plt.xlim(0, FREQ_MAX)  # 周波数範囲を制限
        # y軸の範囲調整: 0dBがピークなので、少し上まで表示すると見やすい
